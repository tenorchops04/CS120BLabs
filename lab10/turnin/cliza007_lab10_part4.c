...
enum SP_States{SP_SMStart, Wait, PA2pressed}SP_state;
void Tick_Speaker(){
	...
	switch(SP_state){
		...
		case PA2pressed:
			k = ~k;
			if(k){
				combineLEDs = (combineLEDs & 0xFF) | 0x10;
			}
			else{
				combineLEDs = combineLEDs & 0x0F;
			}
			break;
	}
}

enum FR_States {FR_Start, Wait2, Inc, IncWait, Dec, DecWait} FR_state;
void Tick_Frequency(){
...
int main(void) {
	...
	unsigned long FR_elapsedTime = 1;
